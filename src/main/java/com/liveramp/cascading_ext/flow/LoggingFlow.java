...
/**
 * Delegates actual flow operations to a flow that gets passed in, but performs
 * some additional logging when the job completes or fails.
 */
public class LoggingFlow implements Flow<JobConf> {
		...
		try {
			...
			for (FlowStepStats stat : stepStats) {
				try {
					...
					// We limit the number of potential logs being pulled to
					// spare the jobtracker
					if (failures.size() > 0) {
	...
	private static String retrieveTaskLogs(TaskAttemptID taskId, String baseUrl) throws IOException {
	...
	public List<String> getSourceNames() {
		return internalFlow.getSourceNames();
	}

	@Override
	public List<String> getSinkNames() {
		return internalFlow.getSinkNames();
	}

	@Override
	public List<String> getTrapNames() {
		return internalFlow.getTrapNames();
	}

	@Override
	public List<String> getCheckpointNames() {
		return internalFlow.getCheckpointNames();
	}
}
